...
namespace Coordinates
{
    static class Ut
    {
        ...
        public static string Fmt(this string formatStr, params object[] args) { return string.Format(formatStr, args); }
        ...
        public static List<T> MinElements<T>(this IEnumerable<T> source, Func<T, int> valueSelector)
        {
            var list = new List<T>();
            int? minSoFar = null;
            foreach (var elem in source)
            {
                ...
                if (minSoFar == null || minSoFar == val)
                {
                    minSoFar = val;
                    list.Add(elem);
                }
                else if (val < minSoFar.Value)
                {
                    list.Clear();
                    minSoFar = val;
                    list.Add(elem);
